...
VideoEncoder::SetEncodeParameters(NVEncoderParams *pParams)
{
    ...
    m_pVideoFrame = new unsigned char[m_nVideoFrameSize+1];
...
size_t
VideoEncoder::ReadNextFrame(/*std::queue<unsigned char*> &queueFrame*/ unsigned char* framedata)
{
	...
	/*if(!queueFrame.empty())
	{
		memcpy(m_pVideoFrame, queueFrame.front(), m_nVideoFrameSize);
		return m_nVideoFrameSize;
	}*/

    //return -1;

	memcpy(m_pVideoFrame, framedata, m_nVideoFrameSize+1);
	return m_nVideoFrameSize+1;
}
