...
// includes, CUDA
// Ghi chu: include dung theo thu tu cac file
...
DWORD WINAPI ThreadCamera( LPVOID lpParam );
...
int main(int argc, char *argv[])
{
	...
	/*HANDLE hwArr[2];
	hwArr[0] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&ThreadCamera, NULL, 0, NULL);
	hwArr[1] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&ThreadEncode, NULL, 0, NULL);
	
	WaitForMultipleObjects(2, hwArr, TRUE, INFINITE);
...
DWORD WINAPI ThreadEncode( LPVOID lpParam )
{
	...
	int iFrameWidth = sEncoderParams.iInputSize[0];
	int iFrameHeidht = sEncoderParams.iInputSize[1];
	int iFrameBuffSize = (int)pCudaEncoder->GetBytesPerFrame();

	printf("Start Camera thread ....\n");
	printf("Thread: %d %d %d\n", iFrameWidth, iFrameHeidht, iFrameBuffSize);

	VideoCapture camera(0);
	camera.set(CV_CAP_PROP_FPS, 30.0);
	camera.set(CV_CAP_PROP_FRAME_WIDTH, (double)iFrameWidth);
	camera.set(CV_CAP_PROP_FRAME_HEIGHT, (double)iFrameHeidht);
	...
	printf("sent to the CUDA H.264 Encoder\n");
	...
	while (true)
	{
		...
		{*/
			...
			size_t bytes_read = pCudaEncoder->ReadNextFrame(dstframe.data);
			...
			{
				printf("Warning, Queue Frame is empty\n");
			}
...
DWORD WINAPI ThreadCamera( LPVOID lpParam )
{
	...
	while(true)
	{
		...
		Mat srcframe, dstframe;
		camera >> srcframe; // get a new frame from camera
		cvtColor(srcframe, dstframe, CV_RGB2YUV);
